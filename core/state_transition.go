...
// distributeGasForSGT distributes the gas according to the priority:
//  1. if refund, first native, then SGT
//  2. otherwise, first SGT, then native
//
// note: the returned values are always non-nil.
func (st *StateTransition) distributeGasForSGT(amount, pool1, pool2 *uint256.Int) (quota1, quota2 *uint256.Int) {
	if amount == nil {
...
func (st *StateTransition) innerTransitionDb() (*ExecutionResult, error) {
	...
	if st.evm.Config.NoBaseFee && msg.GasFeeCap.Sign() == 0 && msg.GasTipCap.Sign() == 0 {
		...
		if st.usedSGTBalance != nil && st.evm.ChainConfig().IsOptimism() && !st.evm.ChainConfig().Optimism.IsSoulBackedByNative {
			fee, _ = st.distributeGasForSGT(fee, st.usedSGTBalance, st.usedNativeBalance)
		}
	...
	if optimismConfig := st.evm.ChainConfig().Optimism; optimismConfig != nil && rules.IsOptimismBedrock && !st.msg.IsDepositTx {
		...
		// otherwise we add to the native balance
		if st.usedSGTBalance != nil && st.evm.ChainConfig().IsOptimism() && !st.evm.ChainConfig().Optimism.IsSoulBackedByNative {
			amtU256, _ = st.distributeGasForSGT(amtU256, st.usedSGTBalance, st.usedNativeBalance)
		}
...
func (st *StateTransition) refundGas(refundQuotient uint64) uint64 {
	...
	if st.usedSGTBalance == nil {
		...
		native, sgt := st.distributeGasForSGT(remaining, st.usedNativeBalance, st.usedSGTBalance)
		if native.Sign() > 0 {
			st.state.AddBalance(st.msg.From, remaining, tracing.BalanceIncreaseGasReturn)
		}
		if sgt.Sign() > 0 {
